...
bool
PinholeCamera::Parameters::readFromJsonFile(const std::string& filename)
{
    Json::Reader reader;
    Json::Value root;
    ...
    if(reader.parse(in, root, false)) {
        ...
        m_modelType = PINHOLE;
        m_cameraName = root[id]["sensor_name"].asString();
        m_imageWidth = root[id]["param"]["img_dist_w"].asInt();
        m_imageHeight = root[id]["param"]["img_dist_h"].asInt();
        ...
        if (dist_col < 4) {
            std::cout << "distortion param must be eqaul to or larger than 4.\n";
            return false;
        }
        m_k1 = d[0][0].asDouble();
        m_k2 = d[0][1].asDouble();
        ...
        m_fx = intri[0][0].asDouble();
        m_fy = intri[1][1].asDouble();
        m_cx = intri[0][2].asDouble();
        m_cy = intri[1][2].asDouble();
    }
    in.close();
