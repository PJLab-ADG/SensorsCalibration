 ...
 */
#include "apriltag/contours.c"
#include "apriltag/corner_detect.hpp"

namespace cameracalib {
namespace apriltag {

bool ApriltagDetector::detect(const std::vector<std::vector<float>> &imgGray,
                              const ApriltagBoard &board_pattern,
...
void ApriltagDetector::filterTooCloseCandidates(
...
std::vector<Point2f>
ApriltagDetector::alignContourOrder(Point2f corner,
...
Eigen::MatrixXf
ApriltagDetector::getPerspectiveTransform(std::vector<Point2f> src,
...
void ApriltagDetector::EstimateGrayThres(
...
void ApriltagDetector::ImageThreshold(
...
void ApriltagDetector::GetContours(
...
void ApriltagDetector::findMarkerContours(
...
void ApriltagDetector::ApproxPolyDP(const std::vector<PointRDP> src_contour,
                                    double eps,
...
void ApriltagDetector::findContours(
    const std::vector<std::vector<bool>> &_in,
    std::vector<std::vector<PointRDP>> &contours) {
  int **image;
...
bool ApriltagDetector::isContourConvex(const std::vector<PointRDP> &_contour) {
...
void ApriltagDetector::warpPerspective(
...
bool ApriltagDetector::getPixel(const std::vector<std::vector<bool>> &_src,
...
int ApriltagDetector::findID(std::vector<bool> ID_detect,
...
int ApriltagDetector::extractBits(
