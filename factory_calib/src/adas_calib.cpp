...
bool CalibrationBoardTool::ApriltagBoardCalibration(
    const std::vector<std::vector<float>> &gray_img,
    const InputParam &calibration_inputs, const std::string &output_json_path,
    CalibrationOutputParams *calibration_result) {
  if (calibration_result == nullptr) {
  ...
  // generate vertical board pattern
  ApriltagBoard apriltag_board;
  // check board pattern
  if (!apriltag_board.check()) {
    LOGE("wrong param");
    return false;
  }

  // corner detect
  apriltag::ApriltagDetector board_detector;
  apriltag::CornerPoints corners;
  bool is_success = board_detector.detect(gray_img, apriltag_board, &corners);
  if (!is_success) {
    LOGE("failed to detect corner");
    return false;
  }
